    ...
    )
{
    ...
    RequestVoteResponseMsg_t rvr;
    memset(&rvr, 0, sizeof(RequestVoteResponseMsg_t));
    rvr.term = 1;
    rvr.vote_granted = 0;
    ...
    )
{
    ...
    RequestVoteMsg_t rv;
    memset(&rv, 0, sizeof(RequestVoteMsg_t));
    rv.term = 1;
    ...
    CuTest * tc)
{
    ...
    memset(&ae, 0, sizeof(AppendEntriesMsg_t));
    ae.term = 1;

    /*  higher current term */
    raft_set_current_term(r, 5);
    AppendEntriesResponseMsg_t aer;
...
void TestRaft_follower_recv_appendentries_increases_log(CuTest * tc)
{
    ...
    ae.leader_commit = 5;
    /* include one entry */
    memset(&ety, 0, sizeof(LogEntryMsg_table_t));
    ety.data.buf = str;
    ety.data.len = 3;
    ety.id = 1;
    ...
    CuTest * tc)
{
    ...
    ae.prev_log_term = 1;
    /* include one entry */
    memset(&mety, 0, sizeof(LogEntryMsg_table_t));
    char *str4 = "444";
    mety.data.buf = str4;
    mety.data.len = 3;
    mety.id = 4;
    ...
    CuTest * tc)
{
    ...
    ae.prev_log_idx = 1;
    ae.prev_log_term = 1;
    LogEntryMsg_table_t e[1];
    memset(&e, 0, sizeof(LogEntryMsg_table_t) * 1);
    e[0].id = 1;
    ae.entries = e;
    ae.n_entries = 1;
    ...
    CuTest * tc)
{
    ...
    ae.prev_log_term = 1;
    /* include entries */
    LogEntryMsg_table_t e[2];
    memset(&e, 0, sizeof(LogEntryMsg_table_t) * 2);
    e[0].id = 1;
    e[1].id = 2;
    ae.entries = e;
    ae.n_entries = 2;
    ...
    CuTest * tc)
{
    ...
    ae.prev_log_term = 1;
    /* include entries */
    LogEntryMsg_table_t e[4];
    memset(&e, 0, sizeof(LogEntryMsg_table_t) * 4);
    e[0].term = 1;
    e[0].id = 1;
    e[1].term = 1;
    e[1].id = 2;
    ...
    CuTest * tc)
{
    ...
    ae.prev_log_term = 1;
    /* include entries */
    memset(&e, 0, sizeof(LogEntryMsg_table_t) * 5);
    e[0].term = 1;
    e[0].id = 2;
    e[1].term = 1;
    e[1].id = 3;
    ...
    CuTest * tc)
{
    ...
    RequestVoteResponseMsg_t vr;
    memset(&vr, 0, sizeof(RequestVoteResponseMsg_t));
    vr.term = 0;
    vr.vote_granted = 1;
...
void TestRaft_candidate_requestvote_includes_logidx(CuTest * tc)
{
    ...
    RequestVoteMsg_t* rv = sender_poll_msg_data(sender);
    CuAssertTrue(tc, NULL != rv);
    ...
    CuTest * tc)
{
    ...
    AppendEntriesMsg_t* ae = sender_poll_msg_data(sender);
    ...
    CuTest * tc)
{
    ...
    memset(&aer, 0, sizeof(AppendEntriesResponseMsg_t));
